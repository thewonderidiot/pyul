...
class AgcPass1(Pass1):
    def __init__(self, mon, yul):
        ...
        self.op_thrs = {
            ...
            'DOUBLE': 0o7002,
            'MM':     0o5740,
            'DNCHAN': self.agc_dnchan,
            'DNPTR':  self.agc_dnptr,
            # CODES 21
            'AD':     0o7000,
            'ADS':    0o5310,
            'ADRES':  0o4340,
            'ECADR':  0o4360,
            'EBANK=': self.agc_ebank,
            'REMADR': 0o4344,
            'VAD':    0o4231,
            'ACOS':   0o5171,
            'ABS':    0o5251,
            ...
            'SETLOC': self.setloc,
            'BANK':   self.agc_bank,
            '2BCADR': self.agc_2bcadr,
            '2CADR':  self.agc_2bcadr,
            '2FCADR': self.agc_2fcadr,
            'BBCON':  0o4364,
            'FCADR':  0o4350,
            'BDSU':   0o4321,
            'BDDV':   0o4221,
            ...
            'WAND':   0o4174,
            'OCTAL':  self.octal,
            'SBANK=': self.agc_sbank,
            '2DNADR': self.agc_2dnadr,
            '6DNADR': self.agc_6dnadr,
    ...
    def agc_sbank(self, popo):
        # SBANK= establishes an S-bank for pass 2 chekcing of references.
        popo.health |= (0o744 << 16)
        popo.health |= HealthBit.CARD_TYPE_NWINST
        return self.nd_setloc(popo)

    def agc_bnksm(self, popo):
        # BNKSUM forms TC self pairs at bank ends.
        popo.health |= (0o750 << 16)
        popo.health |= HealthBit.CARD_TYPE_NWINST
        return self.nd_setloc(popo)

    def agc_dnptr(self, popo):
        popo.health |= (0o607 << 21)
        return self.instruct(popo)

    def agc_dnchan(self, popo):
        return self.agc_dnx(popo, 7)

    def agc_6dnadr(self, popo):
        return self.agc_dnx(popo, 5)

    def agc_5dnadr(self, popo):
        return self.agc_dnx(popo, 4)

    def agc_4dnadr(self, popo):
        return self.agc_dnx(popo, 3)

    def agc_3dnadr(self, popo):
        return self.agc_dnx(popo, 2)

    def agc_2dnadr(self, popo):
        return self.agc_dnx(popo, 1)

    def agc_dnx(self, popo, num):
        popo.health |= (num << 27)
        opcode = self.op_thrs['ECADR'] & ~Bit.BIT37
        return self._op_found(popo, opcode)

    # Special routine in pass 1 for AGC to respond to the finding of an instruction operatio ncode. If
